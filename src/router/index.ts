	...
	let drs = [
		{
			...
			children: [//菜单项
				{//第一个是主页，这里写死加主页 helm
					path: '/home',
					name: 'home',
					//下面是首页，只要修改这里就可以实现主页链接修改
					component: () => import('/@/views/home/index.vue'),
					//component: () => import('/@/views/device/falldowndevice/index.vue'), //sample
					meta: {
...
router.beforeEach(async (to, from, next) => {
	...
	if (to.path === '/login' && !token) {
		...
		if (!token) {//token没有了，那么返回登录页 helm
			next(`/login?redirect=${to.path}&params=${JSON.stringify(to.query ? to.query : to.params)}`);
			Session.clear();
			resetRoute();
			NProgress.done();
		} else if (token && to.path === '/login') { //如果是登录动作，那么进入首页 helm
			next('/home');
			NProgress.done();
		} else {
